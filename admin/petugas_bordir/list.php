                        ...
                        <table class="table table-striped table-bordered table-hover">
                            <thead class="table-light text-center">
                                <tr>
                                    ...
                                    <th style="width: 20%;">Nama Bordir</th>
                            ...
                            <tbody>
                                <?php
                                $sql = "SELECT * FROM bordir ORDER BY nama_bordir";
                                $bordir = query($sql);
                                $no = 1;

                                if (empty($bordir)): ?>
                                    <tr>
                                        <td colspan="7" class="text-center">Tidak ada data bordir</td>
                                    </tr>
                                    <?php else:
                                    foreach ($bordir as $row): ?>
                                        <tr>
                                            ...
                                            <!-- <td class="text-center">
                                                ...
                                                <a href="delete.php?id=<?= $row['id_bordir'] ?>"
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', function(e) {
                ...
                // Check if bordir can be deleted
                fetch(`check_delete.php?id=${id}`)
                    .then(response => response.json())
                    .then(data => {
                        if (data.can_delete) {
                            Swal.fire({
                                title: 'Yakin hapus data bordir?',
