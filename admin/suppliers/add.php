...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("INSERT INTO suppliers (nama, alamat, kontak, email) VALUES (?, ?, ?, ?)");
            $stmt->execute([$nama, $alamat, $kontak, $email]);

            $_SESSION['success'] = "Supplier berhasil ditambahkan";
            header('Location: index.php');
            exit;
        } catch (PDOException $e) {
            $errors[] = "Gagal menambahkan supplier: " . $e->getMessage();
        }
    }
}
?>
